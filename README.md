...
- 菜单栏面板只保留高频操作（保护开关、模式、首选麦克风）和最近 1 条事件摘要；「设置…」打开独立设置窗口（通用 / 高级 / 事件 / 关于）
- 两种保护模式：
...
- 最近事件：菜单栏最多展示最新 1 条摘要；设置 → 事件展示最近 5 条详情。事件只记录已经确认生效的用户选择、Auto 接受与恢复动作，并说明每次自动决策的原因
- 恢复通知开启时，已经确认的麦克风恢复动作可发送本地通知；Manual 模式恢复通知最多 10 秒一条（恢复动作本身不降频）。CoreAudio listener 初次安装失败时按 250ms / 500ms / 1s / 2s / 4s / 8s 进行有限自动恢复；当前安装轮次耗尽后会独立进行系统通知授权检查，并在系统允许时为该轮最多提交一条可靠性告警。listener 失败不会改写用户保存的保护开关；可在菜单或设置中「重新尝试」，该可靠性告警不受「显示通知」开关控制
...
通知开关开启时，正常启动并成功建立 CoreAudio 监听后约 0.5 秒请求通知权限；若某一 listener 安装轮次的 initial + 6 retry 全部失败，则会为该轮可靠性告警立即做一次 fresh authorization check。用户显式「重新尝试」或在 failed 状态下关闭后重新开启保护，可以开启新的 listener 安装轮次；App 重启也会创建新的轮次。被拒绝时设置窗口（通用 → 通知）会显示提示行并可一键跳转系统设置。
...
| `protectionEnabled` | 用户是否希望启用保护；listener 临时/当前轮次安装失败不会自动改写 |
| `protectionMode` | `auto` / `manual` |
| `notificationsEnabled` | 是否显示普通麦克风恢复通知；不影响 listener 安装轮次耗尽的可靠性告警 |
