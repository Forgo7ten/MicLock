...
7. `current == target` 只有来自本轮成功 fresh read 时才可独立于 topology 确认；共享 `currentDevice` cache 绝不能作为 confirmation evidence。Auto 下 Protection restore 的真实 target confirmation 本身也会重新开启/延长 settle，并在提交前把既有 notification rebind 到当前 episode。target offline 只有完整可信 topology 才可判定。Trusted User Selection 使用 latest-wins：新的 MicLock 点击立即 supersede 旧 Trusted transaction 并发出新 setter；旧 setter 的延迟回声、未知第三状态或真实 callback 都不参与 provenance 推断，也不能取消最新 target。短确认窗口耗尽后旧 Trusted transaction 结束；若 expiry 来自 `programmaticSwitchWatchdog`，该 synthetic observation 在 Auto 下到此结束，不参与 stable external-switch classification，也不会创建 `StableExternalSwitchCandidate`。Manual 仍可继续 normal protection policy
...
| Programmatic transaction | origin 是事务唯一语义来源；confirmation 只接受 fresh current observation，读取失败保留的 cache 不可确认；Trusted User Selection 使用 latest-wins，新点击立即 supersede 旧 transaction 并发出新 setter，旧 B/C 延迟回声不能完成或取消最新 D；Trusted 仅 500ms fast retry 一次并在第二个 500ms 窗口后结束，不进入永久 64s retry，也不暴露 Protection 状态；Auto 下 Trusted watchdog expiry 的 synthetic observation 不参与 external-switch learning，而后续真实 default-input callback 仍可正常 candidate → accept；Manual timeout 后仍继续 Protection Restore；Protection restore 的 setter error 保留 transaction 并继续 persistent retry，accepted retry 与 Auto protection 的真实 target confirmation 都会重新锚定 settle；`current == target` 在枚举失败时仍可确认，Protection stuck source 会由 watchdog 主动持续重试且跨过退避阈值后也不会被 Auto 反向学习、超过旧 1s 阈值后的晚确认仍成功 |
